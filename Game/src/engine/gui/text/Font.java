...
public class Font {
	 ...
	 * 
	 * @param fontType - File Path and name of the font
	 */
	public Font(String fontType) {
		...
		try {
			...
			reader = new BufferedReader(
					new InputStreamReader(getClass().getResourceAsStream("/fonts/" + fontType + ".fnt")));
			// Buffer Values
			int character;
			...
			while ((character = reader.read()) != -1) {
				...
				if (word.contains("char ")) {
					...
					Glyph glyph = new Glyph(id, new Vector2f(position), new Vector2f(surface), new Vector2f(offset),
							advance);
	 ...
	 * 
	 * @param reader - the buffer reader in which we read the data from the file
	 * @param word   - A string used as buffer
	 */
