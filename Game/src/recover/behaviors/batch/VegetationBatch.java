 ...
 * @author louis
 */
public class VegetationBatch extends EngineBehavior implements Batch<StaticModel>{
	...
	public VegetationBatch(TerrainModel terrain) {
	...
	public void sizeBuffer() {
		...
		for(StaticModel model : elements) {
			((Buffer)newBuffer).position(model.offset());
			((Buffer)newBuffer).limit(model.offset() + model.size() * PolyModel.VERTEX_BYTE_SIZE);
