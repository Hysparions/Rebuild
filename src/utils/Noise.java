...
public class Noise {
	...
	public double generate(double x, double y, double z) {
		...
		for (int i = 0; i < octave; i++)
		{
			...
			double res = lerp(w, lerp(v, lerp(u, grad(permutation.get(AA), x, y, z), grad(permutation.get(BA), x - 1, y, z)), lerp(u, grad(permutation.get(AB), x, y - 1, z), grad(permutation.get(BB), x - 1, y - 1, z))), lerp(v, lerp(u, grad(permutation.get(AA + 1), x, y, z - 1), grad(permutation.get(BA+1), x - 1, y, z - 1)), lerp(u, grad(permutation.get(AB + 1), x, y - 1, z - 1), grad(permutation.get(BB + 1), x - 1, y - 1, z - 1))));
