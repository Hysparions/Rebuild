...
public class Word {
	...
	private void updateBuffer() {
		...
		for(int i=0 ; i < word.length(); i++) {
			...
			vertices.put(x); vertices.put(y+position.y); 
			vertices.put(color.x); vertices.put(color.y); vertices.put(color.z); 
			vertices.put(glyph.position().x); vertices.put(glyph.position().y+ glyph.surface().y);
			...
			vertices.put(x); vertices.put(y+position.y); 
			vertices.put(color.x); vertices.put(color.y); vertices.put(color.z); 
			vertices.put(glyph.position().x+ glyph.surface().x); vertices.put(glyph.position().y);
			//Third Vertex
			x = cursor + glyph.offset().x*scale;
			y = -glyph.offset().y*scale;
			vertices.put(x); vertices.put(y+position.y); 
			vertices.put(color.x); vertices.put(color.y); vertices.put(color.z); 
			vertices.put(glyph.position().x); vertices.put(glyph.position().y);
			...
			vertices.put(x); vertices.put(y+position.y); 
			vertices.put(color.x); vertices.put(color.y); vertices.put(color.z); 
			vertices.put(glyph.position().x+ glyph.surface().x); vertices.put(glyph.position().y+glyph.surface().y);
	...
	public FloatBuffer buffer() {return vertices;}
